...
library(png)
library(grid)
which_function <- readPNG('which_function.png')
...
This tells us that when we "flipped" our coin 100 times, we got a sample containing `r flips` heads (assuming we define 1 as heads and 0 as tails). More generally speaking, our sample contains `r flips` successes out of 100 (meaning there were `r 100-flips` failures).
...
There is a `r pnorm(q=95, mean=100, sd=15)` probability of sampling a value of **95 or less** from a normal distribution with mean 100 and sd 15. 
...
```{r}
pnorm(q=95, mean=100, sd=15,lower.tail = F)
```
  
There is a `r pnorm(q=95, mean=100, sd=15,lower.tail = F)` probability of sampling a value of **95 or greater** from a normal distribution with mean 100 and sd 15.
  
One final point about `pnorm()`. What do you think happens when we add these two probabilities together?
  
```{r}
pnorm(q=95, mean=100, sd=15)+pnorm(q=95, mean=100, sd=15,lower.tail = F)
```
...
We also get a value of 1 if we set `q` equal to the number of trials.
  
```{r}
pbinom(q=10,size=10,prob=.5)
```
  
This makes sense because there's a 100% chance of flipping a coin 10 times and getting 10 or fewer heads. In other words, this **exhausts** all possibilities.
...
Again, these two probabilities sum to 1.
  
```{r}
ppois(q=1,lambda=2)+ppois(q=1,lambda=2,lower.tail=F)
```
...
Above, we calculate the cumulative probability from 10 to the lower limit (0).
...
We see that 5% of the normal distribution (with mean 100 and sd 15) is to the left of `r qnorm(p=.05,mean=100,sd=15)`. 
...
This tells us that 5% of the binomial distribution (with 10 trials and probability of .5 on a single trial) is to the left of `r qbinom(p=.05,size=10,prob=.5)`. 
...
This tells us that 50% of the Poisson distribution (with `lambda` equal to 2) is to the left of `r qpois(p=.05,lambda=2)`.
